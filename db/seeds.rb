...
=end
name=["fiat panda","mercedes a-class","peugeot 308","nissan micra","vw polo","citroen saxo","opel corsa","mitsubishi asx","daewoo matiz","skoda fabia","audi a3"]
kiliometers=[170500,120150,168025,135658,27580,56325,149058,78965,45895,98654,56478]
fuel_date=["2018-01-04","2018-01-02","2018-01-06","2018-01-07","2018-01-01","2018-01-05","2018-01-09","2018-01-03","2018-01-14","2018-01-05","2018-01-13"]
fuel_type=["Βενζίνη","Πετρέλαιο","Βενζίνη","Πετρέλαιο","Βενζίνη","Βενζίνη","Βενζίνη","Πετρέλαιο","Βενζίνη","Πετρέλαιο","Βενζίνη"]
user_id=[5,6,6,5,7,7,7,6,8,8,4]
station_id=[1041,1061,1061,1041,450,450,442,1061,2044,2046,2044]
area=["Ιωάννινα","Ιωάννινα","Ιωάννινα","Ιωάννινα","Άρτα","Άρτα","Άρτα","Ιωάννινα","Πρέβεζα","Πρέβεζα","Πρέβεζα"]
weeks=365/7.to_i
...
for i in 1..weeks
	for j in 0..name.size-1
		...
		if(i==1)
			...
			Vehicle.create!(name:"#{name[j]}",kilometers:new_km[j],liters:liters,fuel_date:fuel_date[j],fuel_type:"#{fuel_type[j]}",area:"#{area[j]}",user_id:user_id[j],station_id:station_id[j],price:price)
		else
		...
		if(flag)
			...
			Vehicle.create!(name:"#{name[j]}",kilometers:new_km[j],liters:liters,fuel_date:new_date[j],fuel_type:"#{fuel_type[j]}",area:"#{area[j]}",user_id:user_id[j],station_id:station_id[j],price:price)
  	...
  	worksheet.each do |row|
  		...
  		if row
			if (num_rows!=0)
				...
				Vehicle.create!(name:"#{row_cells[0]}",kilometers:row_cells[1],liters:row_cells[2].to_f,fuel_date:row_cells[4],fuel_type:"#{row_cells[5]}",area:"#{row_cells[6]}",user_id:row_cells[8],station_id:row_cells[7],price:row_cells[3])
			else
				num_rows += 1
			end
